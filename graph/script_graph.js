...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 데이터 로드 → 가공 → 필터 → 차트
    function loadData() {
      // tierConfig가 아직 로드되지 않았으면 대기 (실제로는 Promise.all 내에서 호출되므로 보장됨)
      if (!tierConfig) {
    ...
    function createGraph(type) {
      ...
      const avgPickRateChartData = pickRateValuesInChartData.length === 0 ? 0 : pickRateValuesInChartData.reduce((s, pr) => s + pr, 0) / pickRateValuesInChartData.length;
       ...
       // chartData의 '승률', 'TOP 3'는 0-1 스케일로 저장되어 있다고 가정
       ...
       chartData.forEach(d => {
           ...
           if (weight > 0) {
               ...
               wWin_sum += (d['승률'] || 0) * weight;   // calculateFinalStatsForPeriod 결과의 승률 값 사용 (0-1 스케일)
              ...
              callbacks:{
                ...
                label:ctx=>{
                  ...
                  return [
                    ...
                    `승률: ${d['승률'] !== undefined && d['승률'] !== null ? (d['승률'] * 100).toFixed(2) + '%' : '-'}` // 승률은 0-1 비율로 저장되었을 수 있으므로 100 곱함
                  ];
                }
              }
            },
      ...
      myChart.config._가중평균승률 = wWin_chartData * 100; // wWin_chartData는 0-1 스케일이므로 100 곱해서 저장
